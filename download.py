...
@click.option(
    "--stock",
    prompt="Stock listing name",
    help="Stock you want to download"
)
@click.option(
    "--start",
    prompt="Start date YYYY-mm-dd",
    help="Start date you want download stock data",
)
@click.option(
    "--end",
    prompt="End date YYYY-mm-dd",
    help="End date you want download stock data"
)
...
def download(start, end, stock):
    ...
    try:
        ...
        if not "stocklist" in insp.get_table_names():
            ...
            stock.to_sql(
                name="temptable", con=engine, if_exists="replace", index="Date"
            )
