...
def serve():
    ...
    def get_mean():
        ...
        result = engine.execute(
            f"SELECT AVG(High), AVG(LOW), AVG(Open), AVG(Close), AVG(Volume), AVG(AdjClose) FROM stocklist WHERE Name='{target_column}'"
        )
    ...
    def get_datapoints():
        ...
        result = engine.execute(
            f"SELECT COUNT(High) FROM stocklist WHERE Name='{target_column}'"
        )

        for x in result:
            return jsonify({"Number of data points": str(x[0])})
    ...
    def get_peaktopeak():
        ...
        result = engine.execute(
            f"SELECT MAX(High), MIN(Low) FROM stocklist WHERE Name='{target_column}'"
        )
        ...
        return jsonify(
            {
                ...
                + str(round((float(amplitude[0][0]) - float(amplitude[0][1])), 2))
            }
        )
